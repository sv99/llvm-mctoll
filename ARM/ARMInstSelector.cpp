...
SDNode *ARMMachineInstructionRaiser::selectCode(
  ...
  case ARM::t2ADDri12: {
    ...
    if (FrameIndexSDNode::classof(N->getOperand(1).getNode())) {
      ...
      recordDefinition(FuncInfo, Rd.getNode(), Node);
      NPI->Node = Node;
      emitBinary(FuncInfo, BB, ISD::ADD, MI);
    }
  } break;
  /* SUB */
  case ARM::SUBri:
  ...
  case ARM::t2SUBS_PC_LR: {
    ...
    recordDefinition(FuncInfo, Rd.getNode(), Node);
    NPI->Node = Node;
    emitBinary(FuncInfo, BB, ISD::SUB, MI);
  } break;
  /* MOV */
  case ARM::MOVi16:
  case ARM::t2MOVi16:
  ...
  case ARM::t2ANDrs: {
    ...
    recordDefinition(FuncInfo, Rd.getNode(), Node);
    NPI->Node = Node;
    emitBinary(FuncInfo, BB, ISD::AND, MI);
    // TODO:
  ...
  case ARM::t2ASRri: {
    ...
    recordDefinition(FuncInfo, Rd.getNode(), Node);
    NPI->Node = Node;
    emitBinary(FuncInfo, BB, ISD::SRA, MI);
  } break;
  /* CMN */
  case ARM::CMNri:
  case ARM::CMNzrr:
  ...
  case ARM::t2EORri: {
    ...
    recordDefinition(FuncInfo, Rd.getNode(), Node);
    NPI->Node = Node;
    emitBinary(FuncInfo, BB, ISD::XOR, MI);
    // TODO:
  ...
  case ARM::t2MUL: {
    ...
    recordDefinition(FuncInfo, Rd.getNode(), Node);
    NPI->Node = Node;
    emitBinary(FuncInfo, BB, ISD::MUL, MI);
  } break;
  /* MVN */
  case ARM::MVNi:
  case ARM::MVNr:
  ...
  case ARM::t2LSLrr: {
    ...
    recordDefinition(FuncInfo, Rd.getNode(), Node);
    NPI->Node = Node;
    emitBinary(FuncInfo, BB, ISD::SHL, MI);
  } break;
  /* LSR */
  case ARM::LSRi:
  case ARM::LSRr:
  ...
  case ARM::t2LSRrr: {
    ...
    recordDefinition(FuncInfo, Rd.getNode(), Node);
    NPI->Node = Node;
    emitBinary(FuncInfo, BB, ISD::SRL, MI);
  } break;
  /* ORR */
  case ARM::ORRri:
  case ARM::ORRrr:
  ...
  case ARM::t2ORRrs: {
    ...
    recordDefinition(FuncInfo, Rd.getNode(), Node);
    NPI->Node = Node;
    emitBinary(FuncInfo, BB, ISD::OR, MI);
  } break;
  /* ROR */
  case ARM::RORi:
  case ARM::RORr:
